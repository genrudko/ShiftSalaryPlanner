...
M7 starts from canonical `master` `d009796cb6ff179d46327f2228620ae239d3b8f9` with the approved dependency-direction contract: narrow feature-facing ports only where concrete DAO/store/service details leak into presentation orchestration; existing persistence/service implementations stay in place. **Task 1 — ScheduleDataPort is complete locally** at `1b6a7c550a62539a4d597f6d8f3fb108d8001cdd`. **Task 2 — AlarmDataPort + AlarmPlatformPort is complete locally** at `23c328849eea9310a12ded23f819ffb415faba28`. **Task 3 — FinanceDataPort is now complete locally** at `f59c8e1444100f58d5a5c81a7da1776e28119cc5`: the six concrete finance stores (`PayrollSettingsStore`, `WorkplacePayrollSettingsStore`, `AdditionalPaymentsStore`, `DeductionsStore`, `ReportVisibilitySettingsStore`, `ReportHistoryStore`) no longer leak into `ShiftSalaryApp` or `ProfileDependencies`; `FinanceDataPort` exposes their existing flows and exact save/CRUD operations while payroll calculations remain outside the port. Finance targeted tests, M3 payroll characterization and the full JVM are GREEN; current full JVM evidence is **102/102**, zero failures/errors/skips across 28 suites. Next bounded slice: **Task 4 — NotesDataPort + SettingsDataPort + ActivityLogPort**.
