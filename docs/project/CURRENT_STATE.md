...
M7 starts from canonical `master` `d009796cb6ff179d46327f2228620ae239d3b8f9` with the approved dependency-direction contract: narrow feature-facing ports only where concrete DAO/store/service details leak into presentation orchestration; existing persistence/service implementations stay in place. **Task 1 — ScheduleDataPort is complete locally** at `1b6a7c550a62539a4d597f6d8f3fb108d8001cdd`. **Task 2 — AlarmDataPort + AlarmPlatformPort is complete locally** at `23c328849eea9310a12ded23f819ffb415faba28`. **Task 3 — FinanceDataPort is complete locally** at `f59c8e1444100f58d5a5c81a7da1776e28119cc5`. **Task 4 — NotesDataPort + SettingsDataPort + ActivityLogPort is complete locally** at `b4e3367538389bfcad632c3da8eda6d036d891eb`: `ShiftSalaryApp` no longer aliases `AppNotesStore`, `AppWorkflowSettingsStore`, `AssistantAiSettingsStore`, `TodayLayoutSettingsStore`, `PatternTemplatesStore`, or `AppEventLogStore`; existing note/settings/pattern/log semantics remain inside their backing stores and M6 UI state holders remain outside ports. Targeted Notes/Settings/Service/pattern tests and full JVM are GREEN; current full JVM evidence is **104/104**, zero failures/errors/skips across 30 suites. Next bounded slice: **Task 5 — ProfileDataPort**.
