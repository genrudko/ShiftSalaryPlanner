...
M7 starts from canonical `master` `d009796cb6ff179d46327f2228620ae239d3b8f9` with the approved dependency-direction contract: narrow feature-facing ports only where concrete DAO/store/service details leak into presentation orchestration; existing persistence/service implementations stay in place. **Task 1 — ScheduleDataPort is complete locally** at `1b6a7c550a62539a4d597f6d8f3fb108d8001cdd`: `ShiftSalaryApp` has no direct `ShiftDayDao` / `ShiftTemplateDao` / `HolidayDao` / `WorkAssignmentsStore` references and pattern persistence no longer depends on a Room DAO. **Task 2 — AlarmDataPort + AlarmPlatformPort is also complete locally** at `23c328849eea9310a12ded23f819ffb415faba28`: `ProfileDependencies.alarmData` owns alarm persistence, app-level `alarmPlatform` owns scheduler/permission/suppression operations, `MainActivity` and `ShiftAlarmsEffects` have zero direct `ShiftAlarmStore` / `ShiftAlarmScheduler` references, and the shared Wear reschedule helper was adapted without changing Wear persistence behavior. Alarm targeted/structural tests and full JVM are GREEN; current full JVM evidence is **100/100**, zero failures/errors/skips across 26 suites. Next bounded slice: **Task 3 — FinanceDataPort**.
