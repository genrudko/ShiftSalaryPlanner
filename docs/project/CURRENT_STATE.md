...
M7 starts from canonical `master` `d009796cb6ff179d46327f2228620ae239d3b8f9` with the approved dependency-direction contract: narrow feature-facing ports only where concrete DAO/store/service details leak into presentation orchestration; existing persistence/service implementations stay in place. **Tasks 1–4 are complete locally** through `b4e3367538389bfcad632c3da8eda6d036d891eb` (Schedule, Alarm, Finance, Notes/Settings/Activity Log). **Task 5 — ProfileDataPort is complete locally** at `bd1e0bcfbebbea104c098f096a28a33a2c50b73b`: `AppProfileStore` no longer leaks into `ShiftSalaryApp` or `ShiftSalaryPlannerRoot`; profile state and the exact activate/create/rename/delete/clear lifecycle operations are consumed through `ProfileDataPort`, while `createProfileDependencies(context, activeProfileId)` remains composition-root work. The port also supplies the same initial profile snapshot formerly assembled from `AppProfileStore.DEFAULT_*`/`resolveActiveProfileId`, so presentation no longer knows the concrete store. Targeted profile/structural tests and full JVM are GREEN; current full JVM evidence is **106/106**, zero failures/errors/skips across 32 suites. Next bounded slice: **Task 6 — ServiceOperationsPort**.
