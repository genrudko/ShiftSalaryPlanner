...
- [ ] **Step 1:** Create `AppDependencies.kt`. Use `context.applicationContext` for long-lived constructors. `createAppDependencies` creates `AppProfileStore`, `Scope(DriveScopes.DRIVE_APPDATA)`, the current `GoogleSignInClient` configuration, and `ExcelScheduleParser`.
- [ ] **Step 2:** Implement `createProfileDependencies`. It must call `AppDatabase.getDatabase(appContext, activeProfileId)` exactly once, derive all three DAOs from that instance, and create `HolidaySyncRepository(holidayDao)` plus `ExcelScheduleImporter(shiftTemplateDao, shiftDayDao)` from those DAOs. Construct the same 15 profile-keyed stores that the old root constructs.
- [ ] **Step 3:** Run `./gradlew :app:compileDebugKotlin --no-daemon`. Expected: compile success; the new file is not wired yet.
- [ ] **Step 4:** Run `git diff --check` and commit: `refactor: define app dependency boundaries`.
...
- [ ] **Step 1:** Create `ShiftSalaryPlannerRoot.kt` implementing those seven responsibilities and nothing feature-specific.
- [ ] **Step 2:** Change `ShiftSalaryApp` to receive the three new inputs (`profilesState`, `appDependencies`, `profileDependencies`). Remove its `AppProfileStore` construction and profile `stateFlow.collectAsState`; derive `activeProfileId` and `activeProfileName` from supplied `profilesState` exactly as before.
- [ ] **Step 3:** Replace M4-owned constructors inside `ShiftSalaryApp` with aliases from the supplied containers. Preserve the old local variable names (`profileStore`, `payrollSettingsStore`, `shiftDayDao`, `googleDriveScope`, `googleSignInClient`, `excelScheduleParser`, etc.) so downstream code is mechanically unchanged.
- [ ] **Step 4:** Keep `googleSignedInAccount`, activity-result launchers, coroutine/snackbar state, all navigation booleans and all feature `remember*` state in `ShiftSalaryApp`. Those belong to M5/M6.
- [ ] **Step 5:** Run a source-boundary assertion limited to the `ShiftSalaryApp` body and require no occurrences of `AppDatabase.getDatabase(`, `PayrollSettingsStore(`, `ShiftAlarmStore(`, `GoogleSignIn.getClient(`, `Scope(DriveScopes` or `ExcelScheduleImporter(`.
- [ ] **Step 6:** Run `./gradlew :app:testDebugUnitTest --no-daemon`. Expected: all M3 JVM tests pass.
- [ ] **Step 7:** Run `git diff --check` and commit: `refactor: extract application root dependencies`.
...
- [ ] **Step 1:** Replace the current appearance/theme wrapper in `MainActivity.onCreate` with the root call above. Do not change `parseInitialWidgetTab` or `parseInitialWidgetFinanceSubTab` behavior.
- [ ] **Step 2:** Remove only imports made unused by M4; do not broad-format or opportunistically clean the 5k-line file.
- [ ] **Step 3:** Assert the `MainActivity.onCreate` section contains none of `AppearanceSettingsStore(`, `AppDatabase.getDatabase(`, `GoogleSignIn.getClient(` or any `*SettingsStore(` constructor.
- [ ] **Step 4:** Run `./gradlew :app:testDebugUnitTest --no-daemon` and `git diff --check`.
- [ ] **Step 5:** Commit: `refactor: slim Android app entry point`.
...
- [ ] **Step 1:** Re-run source-boundary assertions: `MainActivity.onCreate` does not construct app stores/Room/Google clients; `ShiftSalaryApp` does not construct M4 dependency objects; `ShiftSalaryPlannerRoot` owns profile observation and keyed profile-dependency creation.
- [ ] **Step 2:** Run fresh unit gate: `./gradlew clean :app:testDebugUnitTest --no-daemon`; count JUnit XML and require zero failures/errors/skips.
- [ ] **Step 3:** On the unchanged tree run `./gradlew :app:assembleDebug :wear:assembleDebug :app:lintDebug :wear:lintDebug --no-daemon`; require both APKs and zero lint errors.
- [ ] **Step 4:** Run an independent Codex review of the whole M4 diff against its `master` base. Repair only proven correctness/behavior findings; after any production change repeat affected targeted tests and the final gates.
- [ ] **Step 5:** Update `CURRENT_STATE.md` to `M4 VERIFIED / READY FOR MERGE`, recording exact branch/head and evidence. Append the current TURN END to `WORK_PROGRESS.md` with exact next operation.
